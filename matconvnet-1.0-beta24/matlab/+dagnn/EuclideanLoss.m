...
classdef EuclideanLoss < dagnn.Loss
  ...
  methods
    ...
    function [derInputs, derParams] = backward(obj, inputs, params, derOutputs)
      ...
      derInputs{1}  = derInputs{1}./(size(derInputs{1}, 1) * size(derInputs{1}, 2) * size(derInputs{1}, 3));
      derInputs{2}  = derInputs{2}./(size(derInputs{2}, 1) * size(derInputs{2}, 2) * size(derInputs{2}, 3));
    end
